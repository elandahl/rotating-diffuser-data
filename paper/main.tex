...
still shows finite decay and does not follow the same $\sin\theta$ line, which we
attribute to imperfect on-axis alignment. We summarize what is established by
...
against $\sin\theta$ and compare them with a line through the origin fitted to
the $10^\circ$, $15^\circ$, and $20^\circ$ points only.
...
Figure~\ref{fig:sin} shows the floated slopes versus $\sin\theta$. The
$10^\circ$, $15^\circ$, and $20^\circ$ points fall close to a single line through the
origin,
\begin{equation}
  m(\theta)
  \approx
  k\sin\theta,
  \qquad
  k = (3.57\pm 0.11)\times 10^{3},
\end{equation}
fitted while excluding the nominal $0^\circ$ point. That trend is exactly the
angle factor expected from Eq.~\eqref{eq:jinc} if $\tau_{50}$ tracks the same
combination $\omega\sin\theta$ that sets the jinc argument---even though the
detailed jinc \emph{shape} is not required for the scaling test.

The small-angle point lies well above the $\sin\theta=0$ prediction of zero
slope. Finite decorrelation on axis is expected if the detector is not at true
forward scattering, if the beam is not centered on the rotation axis, or if
boiling/wobble contribute angle-independent dynamics~\cite{Yoshimura1986}. Given
the documented rough alignment of the $0^\circ$ runs, we interpret that point as a
diagnostic of nonideal geometry rather than a failure of the off-axis scaling
law.

\begin{figure}[t]
  \centering
  \includegraphics[width=0.62\textwidth]{figures/fig_slopes_vs_sintheta.pdf}
  \caption{Floated slopes from Fig.~\ref{fig:omega} versus $\sin\theta$. The
  dashed line is a through-origin fit to the $10^\circ$, $15^\circ$, and $20^\circ$
  points only; the nominal $0^\circ$ point is shown but excluded from that fit.}
  \label{fig:sin}
\end{figure}
...
fixed angle and the $\sin\theta$ growth of the rate constant for
...
angle, and (2)~those rates increase approximately proportionally to
$\sin\theta$. A phenomenological erfc$(\ln\tau)$ fit provides stable $\tau_{50}$
values for that test. The near-forward runs remind us that geometry must be
controlled before interpreting small-angle residuals as intrinsic boiling.
